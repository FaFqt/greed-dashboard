 ...
 */
export function useEntitySubscription(client, options) {
    ...
    useEffect(() => {
        ...
        const _subscribe = async () => {
            try {
                ...
                if (!dojoSDK) {
                    const error = new Error('dojoSDK is required for entity subscriptions. Please provide dojoSDK when initializing the MetagameClient, or include worldAddress in your config for automatic dojoSDK creation.');
                    setError(error);
                    logger.warn(error.message);
                    logger.debug('Tip: You can provide worldAddress, relayUrl, and domain in your MetagameConfig for automatic dojoSDK initialization');
                    return;
                }
...
export async function subscribeToEntities(client, options) {
    const { query, namespace, logging = false, transform } = options;
    const entityNamespace = namespace;
    // Check if dojoSDK exists
    const { dojoSDK } = client.getConfig();
    if (!dojoSDK) {
        throw new Error('dojoSDK is required for entity subscriptions. Please provide dojoSDK when initializing the MetagameClient.');
    }
    const state = client.getStore().getState();
    const defaultTransform = (entity) => {
        const { entityId, models } = entity;
