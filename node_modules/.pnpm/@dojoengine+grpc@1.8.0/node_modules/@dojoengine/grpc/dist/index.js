...
var ToriiGrpcClient = class {
  ...
  constructor(config) {
    ...
    this.worldAddress = config.worldAddress ?? void 0;
  ...
  toActivityQueryInput(query) {
    ...
    const hasAdditionalFilters = Boolean(
      query?.namespaces?.length || query?.callerAddresses?.length || query?.fromTime !== void 0 || query?.toTime !== void 0 || query?.pagination
    );
    ...
    if (query?.fromTime !== void 0 && query.fromTime !== null) {
      result.from_time = query.fromTime.toString();
    }
    if (query?.toTime !== void 0 && query.toTime !== null) {
  ...
  async onEntityUpdated(clause, callback, worldAddresses) {
    const worldAddressesBytes = this.normalizeWorldAddresses(worldAddresses);
    return this.createStreamSubscription({
  ...
  async updateEntitySubscription(subscription, clause, worldAddresses) {
  ...
  async onEventMessageUpdated(clause, callback, worldAddresses) {
    const worldAddressesBytes = this.normalizeWorldAddresses(worldAddresses);
    return this.createStreamSubscription({
  ...
  async updateEventMessageSubscription(subscription, clause, worldAddresses) {
    const grpcSubscription = this.findSubscription(subscription);
    if (!grpcSubscription) {
      throw new Error("Subscription not found");
    }
  ...
  async getWorlds(worldAddresses) {
    const response = await this.client.worldClient.worlds({
      world_addresses: worldAddresses ? worldAddresses.map((address) => hexToBuffer3(address)) : []
    }).response;
  ...
  async onAggregationsUpdated(query, callback) {
  ...
  async updateAggregationsSubscription(subscription, query) {
    const grpcSubscription = this.findSubscription(subscription);
    if (!grpcSubscription) {
      throw new Error("Subscription not found");
    }
  ...
  async onActivitiesUpdated(query, callback) {
  ...
  async updateActivitiesSubscription(subscription, query) {
    const grpcSubscription = this.findSubscription(subscription);
    if (!grpcSubscription) {
      throw new Error("Subscription not found");
    }
